...
export const readUsersByUsername: RequestHandler = async (
  req: Request,
  res: Response
) => {
  try {
...
export const readUsersBySearchingUsername: RequestHandler = async (
  req: Request,
  res: Response
) => {
  try {
    console.log("search", req.params.search);
    const users = await UserDao.readUsersBySearchingUsername(
      "%" + req.params.search + "%"
    );
    ...
    console.error(
      "[users.controller][readUsersBySearchingUsername][Error]: ",
      error
    );
    res.status(500).json({
      message: "There was an error when fetching users",
